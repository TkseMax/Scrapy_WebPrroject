    ...
    page.on('response', async (response) => {
        ...
        if (url.includes('api.pcexpress.ca/pcx-bff/api/v1/products/')) {
            ...
            try {
                ...
                singlePdata.url = url; //stores URL for reference
                ...
                console.log(`${singlePdata.name}`);
                console.log(`${singlePdata.price}`);
                console.log(`${singlePdata.com_price}`);
                console.log(`${singlePdata.com_quantity}`);
                console.log(`${singlePdata.com_unit}`);
...
function readLinksFile() {
    const readpath = './LINKS/product_links_6.json'; //<<<<NEED TO Change line
    const filePath = path.join(__dirname, readpath); //<<<<NEED TO Change line
...
//     const filePath = path.join(__dirname, 'product_details_pupp.csv');
//     try {
...
async function produrlrun() {
    ...
    try {
        ...
        for (let url of productsLink) {
            const productData = await getProductDetails(url);
            //let str = productData.replace('\r', '').replace('\n', '');
            //let storeddata = str.split("\n");
        ...
        jsonexport(newProductDate, function(err, csv){
            if (err) return console.error(err);
            console.log(csv);
            //console.log(oldData);
            //saving result to a file
            //const combProductDetails = [oldData.concat(csv)];
            //console.log(combProductDetails);
            //fs.writeFileSync('product_details_pupp.csv', combProductDetails);
            const writepath = './PROD_DET/product_details_pupp_6.csv'
            fs.writeFileSync(writepath, csv); //<<<<< This need to change
            console.log(`Results have been saved to : ${writepath}. Total products: ${csv.length}`);
        });
