    ...
    page.on('response', async (response) => {
        ...
        if (url.includes('api.pcexpress.ca/pcx-bff/api/v1/products/')) {
            ...
            try {
                ...
                let com_price = jsonData.offers ? `${jsonData.offers[0].comparisonPrices[0].value}` : 'Comparing Price unavailable';
                let com_quantity = jsonData.offers ? `${jsonData.offers[0].comparisonPrices[0].quantity}` : 'Comparing Quantiy unavailable';
                let com_unit = jsonData.offers ? `${jsonData.offers[0].comparisonPrices[0].unit}` : 'Comparing unit not available';

                //Switch statement to make it standard of 100 gram || 100 ml || 1 EACH
                switch (com_unit) {
                    case "kg":
                        com_quantity *= 10;
                        com_price /= com_quantity;
                        com_quantity = 100;
                        com_unit = "g";
                        break;
                    case "l":
                ...
                console.log(`Comparasion Quantity: ${com_quantity}`);
